...
export default function Header({ triggerRefresh = 0, onMonthChange }: Readonly<HeaderProps>) {
	...
	return (
		<div className="w-full h-[10vh] bg-white dark:bg-[#1c1c1e] border-b border-gray-300 dark:border-gray-700 flex justify-between items-center px-6">
			<div>
				<div className="text-lg font-bold text-gray-800 dark:text-gray-200">{title}</div>
			</div>
			{/* Month navigation */}
			{showMonthNavigationSection && (
				<div className="flex items-center gap-2">
					...
					<span className="text-gray-800 dark:text-gray-200 font-semibold">{formattedMonth}</span>
			...
			<div className="relative">
				<div className="h-full flex items-center">
					<button
						...
						className="px-4 py-2 rounded flex justify-center items-center gap-x-2 text-sm text-white bg-[#294a6f]"
					>
						<Landmark size={25} />
				...
				{open && (
					<div
						ref={popupRef}
						className="absolute right-0 mt-2 w-64 bg-white dark:bg-[#1c1c1e] border border-gray-300 dark:border-gray-700 rounded-xl shadow-lg p-4 z-20"
					>
						<h3 className="font-semibold mb-2 text-gray-800 dark:text-gray-200">Update Net Worth</h3>
						<input
							type="number"
