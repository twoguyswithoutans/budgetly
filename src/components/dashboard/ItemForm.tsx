...
export default function ItemForm({ editable, addAmount, currentDate, isRepeat, onChangeAction, onAddAmountAction, onToggleRepeatAction, onCompleteAmountAction }: Readonly<ItemFormProps>) {
	...
	return (
		<>
			...
			<button onClick={onCompleteAmountAction} className="mb-3 py-2 font-bold gap-x-1 w-full flex justify-center items-center rounded bg-green-100 text-green-700 active:bg-green-200">
				Complete Amount <HandCoins size={20} />
			</button>

			<div className="flex gap-x-2 items-center justify-start">
				<button onClick={onToggleRepeatAction} className={`relative flex h-6 w-11 items-center rounded-full transition-colors ${isRepeat ? "bg-blue-800" : "bg-gray-300 dark:bg-gray-600"}`}>
